...
Model architecture 는 Fig. 2 에 나타난 것처럼 5 개의 stage 로 구성되며, 처음 세 stage 는 RepMixer block 을 사용하고 마지막 두 stage 는 multi-headed self-attention block 을 사용한다. 각 stage 에서의 model depth 는 [2, 12, 24, 4, 2] 이고, 각 stage 의 embedding dimension 은 [96, 192, 384, 768, 1536] 이다. ConvFFN layer 의 MLP expansion ratio 는 4.0 으로 설정된다. 이 model 은 125.1M parameter 를 가지며, 이는 MobileCLIP 의 가장 큰 FastViT variant 보다 3.5→ 크지만, 여전히 널리 사용되는 ViT 대안들보다 작다.
