...
* 구체적으로, 주어진 sentence $t = \{\text{word}_1, \text{word}_2, \ldots, \text{word}_{|t|}\}$ 에 대해, prefix language model $P*\theta$ 는 fixed CLIP text encoder 로부터 추출된 text embedding 에 조건부로 $t$ 를 reconstruct 하도록 학습된다. 
